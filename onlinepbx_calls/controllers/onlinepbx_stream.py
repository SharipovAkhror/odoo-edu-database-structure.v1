# onlinepbx_calls/controllers/onlinepbx_audio.py
# -*- coding: utf-8 -*-
import logging
import requests
...
class OnlinePBXAudioController(http.Controller):

    @http.route('/onlinepbx/stream/<int:call_id>', type='http', auth='user',
                methods=['GET', 'HEAD'])
    def stream(self, call_id, **kw):
        """Proxy OnlinePBX recording with HTTP Range support."""
        call = request.env['onlinepbx.call'].sudo().browse(call_id)
        if not call.exists():
            return request.not_found()

        src_url = (call.download_url or call.recording_url or '').strip()
        if not src_url:
        ...
        headers_in = {
            ...
            "Accept": "*/*",
            "User-Agent": "Odoo-OnlinePBX-Proxy",
        }
        ...
        try:
            ...
            _logger.exception("OnlinePBX stream failed: %s", e)
            return request.make_response(
    ...
    @http.route('/onlinepbx/player/<int:call_id>', type='http', auth='user')
    def player(self, call_id, **kw):
        """Small HTML page with <audio> tag."""
        call = request.env['onlinepbx.call'].sudo().browse(call_id)
        if not call.exists():
            return request.not_found()

        stream_url = f"/onlinepbx/stream/{call_id}"
        html = f"""<!DOCTYPE html>
<html>
<head><meta charset="utf-8">
<title>Call {call.name}</title>
<style>
